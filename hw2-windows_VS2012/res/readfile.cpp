            ...
            if ((str.find_first_not_of(" \t\r\n") != string::npos) && (str[0] != '#')) {
                ...
                if (cmd == "light") {
                    if (numused == numLights) { // No more Lights 
                        ...
                        if (validinput) {
							...
							lightcolor[4 * numused + 7] = values[7];
                            ++numused; 
                        }
                    }
                }
                ...
                // Note that no transforms/stacks are applied to the colors. 
                    ...
                    if (validinput) {
						...
						vec3 local_z = vec3(values[0] - values[3], values[1] - values[4], values[2] - values[5]);
						vec3 local_up = vec3(values[6], values[7], values[8]);
						local_up = Transform::upvector(local_up, local_z);
                ...
                // Also look at demo.txt to get a sense of why things are done this way.
                    ...
                    if (validinput) {
						...
						mat4 local_trans = Transform::translate(values[0], values[1], values[2]);
						rightmultiply(local_trans, transfstack);
                    }
                }
                else if (cmd == "scale") {
                    validinput = readvals(s,3,values); 
                    if (validinput) {
                        ...
                        // You might want to use helper functions on top of file.  
                        // Also keep in mind what order your matrix is!
						mat4 local_scale = Transform::scale(values[0], values[1], values[2]);
						rightmultiply(local_scale, transfstack);
                    }
                }
                else if (cmd == "rotate") {
                    validinput = readvals(s,4,values); 
                    if (validinput) {
						...
						mat3 local_rot = Transform::rotate(values[3],axis);
						mat4 local_rot4 = mat4( vec4(local_rot[0],0),
							vec4(local_rot[1],0),
							vec4(local_rot[2],0),
							vec4(0,0,0,1));
						rightmultiply(local_rot4, transfstack);
                    }
                }
